...
#             
#       PLANNED UPGRADES: - support for gromacs 5
#                         - Comparison in R
#
#------------------------------------------------------------------------------
#
...
sasAnalysis-sites() {
   ...
   cd sas_sites_$name1
   (echo "Protein"; echo "G6P") | $groPATH/$g_sas -s ../$tpr -f ../$trj -n ../g6pd.ndx    \
   ...
   (echo "Protein"; echo "Co-enzyme") | $groPATH/$g_sas -s ../$tpr -f ../$trj   \
      -n ../g6pd.ndx -o $name1"_coenzyme_area.xvg"                             \
  ...
  (echo "Protein"; echo "strNADP+") | $groPATH/$g_sas -s ../$tpr -f ../$trj     \
    -n ../g6pd.ndx -o $name1"_strNADP+_area.xvg"                               \
...
while getopts "hu:b:n:t:s:f:c:e:g:" opt; do
 case $opt in
    h) helpMessage; exit    ;;
    u) unres=$OPTARG        ;;
    b) cpu=$OPTARG          ;;
...
--------------------------------------------------------------------
  ...
  case $choice in
    all|emin|nvt|npt|h20|cond|rmsdfg|cluster|pca|sas|sas-sites|dssp|hb|hb-sites|ggplot|ggplot-bis)
      if [ -z ${timens} ]; then
        timens="X"
